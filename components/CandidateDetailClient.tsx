...
export default function CandidateDetailClient({ initial, user }: { initial: Candidate; user: User }) {
  ...
  const { streaming, final, error, autoMatchFailed } = useCandidateStream(initial);
  ...
  const headerCompany = streaming ? (streaming.works?.[0]?.company ?? null) : c.company;
  const headerSchool = streaming ? (streaming.educations?.[0]?.school ?? null) : c.school;
  const headerDegree = streaming ? (streaming.educations?.[0]?.degree ?? null) : c.degree;
...
function PdfCard({ candidateId, isStreaming, pdfPages }: { candidateId: string; isStreaming: boolean; pdfPages?: number | null }) {
    ...
    <>
      <Card style={{ padding: 0, overflow: 'hidden', width: 230 }}>
        ...
        <div style={{
          ...
          PDF 原文
          {pdfPages != null && (
            <span style={{ fontSize: 10, color: 'var(--fg-subtle)', fontWeight: 400, letterSpacing: 0 }}>
              · {pdfPages} 页
            </span>
          )}
          {isStreaming && (
            <span style={{ marginLeft: 'auto', fontSize: 10, color: 'var(--accent)', fontWeight: 600, letterSpacing: '0.06em' }}>
              解析中
            </span>
          )}
        </div>
        ...
        <div
          style={{ position: 'relative', width: 230, height: PDF_THUMB_H, overflow: 'hidden', background: '#fff' }}
        ...
        {!loaded && (
          <div style={{
            ...
            <div style={{ ...docSkel, height: 10, width: '55%', marginBottom: 18 }} />
            {[88, 72, 82, 65, 90, 70, 60, 78].map((w, i) => (
              <div key={i} style={{ ...docSkel, height: 7, width: `${w}%`, marginBottom: 7, animationDelay: `${i * 0.07}s` }} />
            ))}
            <div style={{ height: 14 }} />
            <div style={{ ...docSkel, height: 9, width: '40%', marginBottom: 12 }} />
            {[85, 68, 75, 55].map((w, i) => (
              <div key={i} style={{ ...docSkel, height: 7, width: `${w}%`, marginBottom: 7, animationDelay: `${(i + 8) * 0.07}s` }} />
            ))}
          </div>
        )}
        ...
        {isStreaming && loaded && (
          <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
            <div style={{
              ...
              background: 'linear-gradient(180deg, rgba(120,80,255,0.04) 0%, transparent 40%, rgba(60,160,255,0.03) 70%, rgba(220,100,255,0.04) 100%)',
            }} />
            <div style={{
              position: 'absolute', left: 0, right: 0, height: '34%',
              background: 'linear-gradient(to bottom, transparent 0%, rgba(180,140,255,0.06) 20%, rgba(255,255,255,0.5) 44%, rgba(160,230,255,0.28) 56%, rgba(210,160,255,0.06) 80%, transparent 100%)',
              animation: 'sift-mirror 3.5s ease-in-out infinite',
            }} />
            <div style={{
              position: 'absolute', left: 0, right: 0, height: '18%',
              background: 'linear-gradient(to bottom, transparent 0%, rgba(200,180,255,0.16) 50%, transparent 100%)',
              animation: 'sift-mirror 3.5s ease-in-out infinite',
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--fg)' }}>PDF 原文</div>
            <Btn
              size="sm"
              variant="ghost"
              icon={<I.X size={16} />}
...
function StreamingScroller({
  streaming, isStreaming, children,
}: {
  ...
  return (
    <div ref={scrollRef} style={{ flex: 1, padding: '20px 28px', overflow: 'auto' }}>
      {children}
...
function StreamingSections({
  c, streaming, isStreaming, onUpdateStatus, candidateId, autoMatchFailed,
  headerBasic, headerTargetRole, headerRole, headerCompany, headerSchool, headerDegree, headerSchoolTier, headerYears, headerSummary,
}: {
  c: Candidate;
  streaming: ReturnType<typeof useCandidateStream>['streaming'];
  isStreaming: boolean;
  ...
  const selfEval = typeof src.selfEvaluation === 'string' ? src.selfEvaluation : null;
  ...
  const Skel = ({ w = '100%', h = 12 }: { w?: string; h?: number }) => (
    <div style={{ height: h, width: w, marginBottom: 6, ...skelStyle }} />
  );
  const SkelSection = ({ rows = 3, title = false }: { rows?: number; title?: boolean }) => (
  ...
  const cardAnim = (delay = 0): React.CSSProperties => ({
  ...
  return (
    <div style={{ display: 'flex', gap: 20, width: '100%', alignItems: 'start' }}>
      ...
      <div style={{ flexShrink: 0, position: 'sticky', top: 0, height: 'fit-content', display: 'flex', flexDirection: 'column', gap: 16 }}>
        <PdfCard candidateId={candidateId} isStreaming={isStreaming} pdfPages={c.pdfPages} />
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', minHeight: 28 }}>
              ...
              {headerBasic.age != null && (
                <span style={{ fontSize: 12, color: 'var(--fg-subtle)', fontWeight: 400, lineHeight: 1 }}>{headerBasic.age} 岁</span>
              )}
            ...
            {headerTargetRole && (
              <div style={{ marginTop: 6 }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '2px 8px', borderRadius: 4, fontSize: 12, fontWeight: 500, background: 'var(--accent-bg-subtle)', color: 'var(--accent-700)' }}>
            ...
            {(headerRole || headerCompany || headerSchool) && (
              <div style={{ display: 'flex', gap: 16, marginTop: 8, fontSize: 13, color: 'var(--fg-muted)', flexWrap: 'wrap', alignItems: 'center' }}>
                {(headerRole || headerCompany) && (
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                    <I.Briefcase size={13} style={{ flexShrink: 0, color: 'var(--fg-subtle)' }} />
                    {headerRole}{headerRole && headerCompany ? ' · ' : ''}{headerCompany}
                    {headerYears != null && (
                      <span style={{ marginLeft: 4, color: 'var(--fg-subtle)' }}>{headerYears} 年</span>
                    )}
                  </span>
                )}
                {headerSchool && (
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                    <I.School size={13} style={{ flexShrink: 0, color: 'var(--fg-subtle)' }} />
                    {headerSchool}{headerDegree ? ` · ${headerDegree}` : ''}
                    {headerSchoolTier && <SchoolTierBadge tier={headerSchoolTier} />}
                  </span>
                )}
              </div>
            )}
            ...
            {(headerBasic.city || headerBasic.email || headerBasic.phone) && (
              <div style={{ display: 'flex', gap: 14, marginTop: 6, fontSize: 12, color: 'var(--fg-subtle)', flexWrap: 'wrap', alignItems: 'center' }}>
                {headerBasic.city && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}><I.MapPin size={12} />{headerBasic.city}</span>}
                {headerBasic.email && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}><I.Mail size={12} />{headerBasic.email}</span>}
                {headerBasic.phone && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}><I.Phone size={12} />{headerBasic.phone}</span>}
              </div>
            )}
            ...
            {(headerSummary || isStreaming) && (
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--border)' }}>
                <div style={{ fontSize: 11, color: 'var(--fg-subtle)', textTransform: 'uppercase', letterSpacing: '0.04em', fontWeight: 500, marginBottom: 8 }}>AI 总结</div>
                {!headerSummary && isStreaming ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                    <div style={{ height: 11, width: '90%', borderRadius: 5, background: 'linear-gradient(90deg, var(--bg-sunken) 25%, var(--bg-elevated) 50%, var(--bg-sunken) 75%)', backgroundSize: '200% 100%', animation: 'sift-shimmer 1.6s ease-in-out infinite', marginBottom: 6 }} />
                    <div style={{ height: 11, width: '75%', borderRadius: 5, background: 'linear-gradient(90deg, var(--bg-sunken) 25%, var(--bg-elevated) 50%, var(--bg-sunken) 75%)', backgroundSize: '200% 100%', animation: 'sift-shimmer 1.6s ease-in-out infinite', marginBottom: 6 }} />
                    <div style={{ height: 11, width: '85%', borderRadius: 5, background: 'linear-gradient(90deg, var(--bg-sunken) 25%, var(--bg-elevated) 50%, var(--bg-sunken) 75%)', backgroundSize: '200% 100%', animation: 'sift-shimmer 1.6s ease-in-out infinite' }} />
                  </div>
                ) : (
                  <p style={{ fontSize: 13, lineHeight: 1.6, color: 'var(--fg-muted)', margin: 0, whiteSpace: 'pre-wrap' }}>
                    {headerSummary || '—'}
                  </p>
                )}
              </div>
        ...
        {/* 教育背景 */}
        <Card style={{ padding: 0, overflow: 'hidden', ...cardAnim(0) }}>
          <div style={{ padding: '14px 20px 12px', fontSize: 14, fontWeight: 600, color: 'var(--fg)' }}>教育背景</div>
          {edus.length === 0 ? (
            <div style={{ padding: '0 20px 16px' }}>
              {isStreaming ? <SkelSection rows={2} /> : <div style={{ fontSize: 13, color: 'var(--fg-subtle)' }}>{placeholder}</div>}
            </div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: 'var(--bg-sunken)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
              ...
              <tbody>
                {edus.map((e: any, i: number) => (
                  <tr key={i} style={{ borderBottom: i < edus.length - 1 ? '1px solid var(--border)' : 'none' }}>
                    <td style={{ padding: '10px 20px', verticalAlign: 'middle' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                        <span style={{ fontWeight: 600, color: 'var(--fg)' }}>{e.school || '—'}</span>
                        <SchoolTierBadge tier={e.schoolTier} />
                      </div>
                    </td>
                    <td style={{ padding: '10px 20px', verticalAlign: 'middle', color: 'var(--fg-muted)' }}>
                      {e.major || '—'}
                    </td>
                    <td style={{ padding: '10px 20px', verticalAlign: 'middle', color: 'var(--fg-muted)', whiteSpace: 'nowrap' }}>
                      {e.degree || '—'}
                    </td>
                    <td style={{ padding: '10px 20px', verticalAlign: 'middle', color: 'var(--fg-subtle)', whiteSpace: 'nowrap', fontFamily: 'var(--font-mono)', fontSize: 12 }}>
                      {e.startDate || ''}{(e.startDate || e.endDate) ? ' — ' : ''}{e.endDate || (e.startDate ? '至今' : '—')}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        {/* 工作经历 */}
        <Card style={{ padding: 20, ...cardAnim(60) }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--fg)', marginBottom: 14 }}>工作经历</div>
          {works.length === 0 ? (
            isStreaming ? <SkelSection rows={4} /> : <div style={{ fontSize: 13, color: 'var(--fg-subtle)' }}>{placeholder}</div>
          ) : (
        ...
        {/* 项目经历 */}
        <Card style={{ padding: 20, ...cardAnim(120) }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--fg)' }}>项目经历</div>
            {(src as any).projectEvalFailed && (
              <span style={{ fontSize: 11, color: 'var(--warn-700)', background: 'var(--warn-100)', border: '1px solid var(--warn-300)', borderRadius: 4, padding: '1px 6px' }}>
                AI 评估暂不可用
              </span>
            )}
          </div>
          {prjs.length === 0 ? (
            isStreaming ? <SkelSection rows={3} title /> : <div style={{ fontSize: 13, color: 'var(--fg-subtle)' }}>{placeholder}</div>
          ) : (
        ...
        {(selfEval || isStreaming) && (
          <Card style={{ padding: 20, ...cardAnim(220) }}>
            <SectionLabel>个人评价</SectionLabel>
            {!selfEval && isStreaming
              ? <SkelSection rows={3} />
              : <p style={{
                fontSize: 13, lineHeight: 1.7, color: 'var(--fg-muted)', margin: 0,
                padding: '10px 12px', background: 'var(--bg-sunken)', borderRadius: 8,
                borderLeft: '3px solid var(--accent-200)', whiteSpace: 'pre-wrap',
              }}>
                {selfEval}
              </p>
            }
